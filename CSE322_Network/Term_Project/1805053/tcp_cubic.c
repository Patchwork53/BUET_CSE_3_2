...
#define HYSTART_CRTT_MAX    100000  //srtt is 100ms = 100000us
#define HYSTART_CWND_MIN	4
...
static int hystart_ack_delta_us __read_mostly = 4000;
...
static inline u32 clamp(const u32 value, const u32 low, const u32 high) {
...
static inline u64 bictcp_clock_us(const struct sock *sk)
{
	return ktime_get_real/1000;
}

static inline void hystart_reset(struct sock *sk)
{	
...
static inline void bictcp_update(struct bictcp *ca, u32 cwnd)
{
	...
	t = ((tcp_time_stamp + (ca->delay_min>>3) - ca->epoch_start) << BICTCP_HZ) / HZ;
...
static void hystart(struct sock *sk)
{
	...
	measure_delay(sk); //recalc ca->delay_min
		
	/* first detection parameter - ack-train detection */
	if ((s32)(now - ca->last_ack) <= hystart_ack_delta_us) {
		ca->last_ack = now;
		
		threshold = ca->delay_min>>1;
...
static void bictcp_acked(struct sock *sk, u32 cnt, ktime_t last)
{
	...
	// change 1
	if (!ca->found && tp->snd_cwnd <= tp->snd_ssthresh ){
		hystart(sk);
	}
